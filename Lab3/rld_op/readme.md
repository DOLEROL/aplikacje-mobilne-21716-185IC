# aplikacje-mobilne-21716-185IC Lab2 - responsywne layouty + nawigacja

## kod programu

### 1) Kod pliku App/Home.js
## Sortowanie i filtorwanie liczb.
![Alt Text](pliki/kod/obr1.png)

![Alt Text](pliki/kod/obr2.png)

### 2) Kod pliku App/Page1.js
### Renderowanie dynamicznie importowanych komponentów
![Alt Text](pliki/kod/obr3.png)

### 3) Kod pliku App/LazyData.js
## Funkcja generująca ciąg znaków o długości 5 - 10.
![Alt Text](pliki/kod/LazyData.png)

### 4) Kod pliku App/Page2.js
## Progres krokowy zawierający 5 kroków.
![Alt Text](pliki/kod/obr4.png)

![Alt Text](pliki/kod/obr5.png)

### 5) Kod pliku App/LoremImpus.js
## Jakiś losowy tekst z internetu
![Alt Text](pliki/kod/LoremImpus.png)

### przykład działania aplikacji
## Strona startowa: wyświtlenie posortowanych i przefiltorwanych 100 pseudolosowych liczb, z zakresu 0 - 1000.

![Alt Text](pliki/apk/obr1.png)

## Druga strona: (lazy loading) wyświetlenie danych dopiero po ich całkowitym wczytaniu na stronę, w między czasie wyświtlanie obrazka ładowania.

![Alt Text](pliki/apk/obr2.png)

![Alt Text](pliki/apk/obr3.png)

## Trzecia strona: progres krokowy.

![Alt Text](pliki/apk/obr4.png)

![Alt Text](pliki/apk/obr5.png)

![Alt Text](pliki/apk/obr6.png)

## Na ostatnim kroku wyświtla się przycisk `Submit`, po jego wciśnięciu wyskakuje `Alert`.

![Alt Text](pliki/apk/obr7.png)